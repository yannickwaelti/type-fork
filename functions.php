...
function type_scripts()
{
	...
	// Theme stylesheet
	wp_enqueue_style('type-style', get_stylesheet_uri(), array(), '1.1.2');
...
function type_excerpt_more($more)
{
	...
	if (get_theme_mod('show_read_more', 1)) {
		$more = sprintf(
			'<span class="read-more-link"><a class="read-more" href="%1$s">%2$s</a></span>',
...
function allchinareviews_social_sharing_buttons($content)
{
	global $post;
	if ((is_singular() && !is_page()) || is_home()) {

		// Get current page URL 
		$allchinareviewsURL = urlencode(get_permalink());

		// Get current page title
		$allchinareviewsTitle = htmlspecialchars(urlencode(html_entity_decode(get_the_title(), ENT_COMPAT, 'UTF-8')), ENT_COMPAT, 'UTF-8');
		// $allchinareviewsTitle = str_replace( ' ', '%20', get_the_title());

		// Get Post Thumbnail for pinterest
		$allchinareviewsThumbnail = wp_get_attachment_image_src(get_post_thumbnail_id($post->ID), 'full');

		// Construct sharing URL without using any script
		$twitterURL = 'https://twitter.com/intent/tweet?text=' . $allchinareviewsTitle . '&amp;url=' . $allchinareviewsURL . '&amp;via=AllChinaReviews';
		$facebookURL = 'https://www.facebook.com/sharer/sharer.php?u=' . $allchinareviewsURL;
		$pinterestURL = 'https://pinterest.com/pin/create/button/?url=' . $allchinareviewsURL . '&amp;media=' . $allchinareviewsThumbnail[0] . '&amp;description=' . $allchinareviewsTitle;
		$whatsappURL = 'whatsapp://send?text=' . $allchinareviewsTitle . ' ' . $allchinareviewsURL;
		...
		$content .= '<div class="allchinareviews-social">';
		$content .= '<h5>SHARE ON</h5> <a class="socicon-twitter allchinareviews-link allchinareviews-twitter" href="' . $twitterURL . '" target="_blank"></a>';
		$content .= '<a class="socicon-facebook allchinareviews-link allchinareviews-facebook" href="' . $facebookURL . '" target="_blank"></a>';
		$content .= '<a class="socicon-pinterest allchinareviews-link allchinareviews-pinterest" href="' . $pinterestURL . '" data-pin-custom="true" target="_blank"></a>';
		$content .= '<a class="socicon-whatsapp allchinareviews-link allchinareviews-whatsapp" href="' . $whatsappURL . '" target="_blank"></a>';
		$content .= '</div>';
